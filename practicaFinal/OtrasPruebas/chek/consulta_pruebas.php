...
$y = $_POST['numero_solicitud'];
	echo $_POST['numero_solicitud'];
	$comprobar_cadena = "SELECT * FROM solicitud WHERE numero_solicitud=$y";
      
	$comprobar_consulta = mysql_query($comprobar_cadena);
	
	$informacion_cliente = mysql_fetch_array($comprobar_consulta);
	$usuariosql = "SELECT * FROM usuario WHERE idUsuario=".$informacion_cliente['idUsuario'];
...
if(isset($_POST['visualizar']))
{
	...
	$password="nucleoduro";
	$ruta="www.programacionweb.virrueta/Evenged/CristianDesigner/Pedidos/".$_POST['numero_solicitud']."/Pruebas";
...
if(isset($_POST['notificar'])){
	...
	for($j=0; $j<count($opcionesEscogidas); $j++){
		$aux = $j+1;
	$send[] = "Opcion ".$aux." ".$opcionesEscogidas[$j]." www.programacionweb.virrueta.org/Avenged/CristianDesigner/Pedidos/".$_POST['numero']."/Pruebas/".$opcionesEscogidas[$j]."\n";
		...
		//echo "La cadena queda asi: ".$cadena;
					...
					$asunto = "Atender Solicitud: ".$_POST['numero'];
